...
import 'admin_video_albums_screen.dart';

class AdminVideoAlbumFormScreen extends ConsumerStatefulWidget {
  final VideoAlbum? album;

  const AdminVideoAlbumFormScreen({super.key, this.album});

  @override
  ConsumerState<AdminVideoAlbumFormScreen> createState() =>
      _AdminVideoAlbumFormScreenState();
}

class _AdminVideoAlbumFormScreenState
    extends ConsumerState<AdminVideoAlbumFormScreen> {
  ...
  Future<void> _submit() async {
    ...
    final provider = ref.read(adminVideoAlbumProvider);
    final now = DateTime.now();

    final newAlbum = VideoAlbum(
      id: widget.album?.id ?? '',
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            widget.album == null
                ? l10n.adminCreated(l10n.adminVideoAlbum.toLowerCase())
                : l10n.adminUpdated(l10n.adminVideoAlbum.toLowerCase()),
          ),
        ),
      );
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    final isLoading = ref.watch(adminVideoAlbumProvider).isLoading;

    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.album == null
              ? l10n.adminNew(l10n.adminVideoAlbum)
              : l10n.adminEdit(l10n.adminVideoAlbum),
        ),
      ),
      body: isLoading
